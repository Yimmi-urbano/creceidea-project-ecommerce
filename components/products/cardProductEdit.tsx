...
import { EditProductIcon, MiniEyeIcon, MiniTrashIcon, ShareIcon } from '../icons';
      ...
      {products.map((item) => (
        <Card key={item._id} className="w-full rounded-lg flex flex-row border-none bg-background/70 dark:bg-sky-950/30">
          <div className="flex items-center gap-4 p-4 flex-grow">
            <img src={item.image_default} alt={item.title} className="w-28 h-28 rounded-xl object-cover" />
            <div>
          ...
          <div className="flex flex-col md:flex-row  justify-center items-center gap-1 md:gap-3 pr-4">
            ...
            <Button isIconOnly color="danger" variant="flat" className="p-0 min-w-7 w-7 h-7 rounded-md" aria-label="Eliminar">
              <MiniTrashIcon size={19} />
            </Button>
            <Button isIconOnly color="primary" variant="flat" className="p-0 min-w-7 w-7 h-7 rounded-md" aria-label="Eliminar">
              <ShareIcon size={19} />
            </Button>
            <Button isIconOnly color="default" variant="flat" className="p-0 min-w-7 w-7 h-7 rounded-md" aria-label="Eliminar">
              <EditProductIcon size={19} />
            </Button>
          </div>
        </Card>
      ))}
