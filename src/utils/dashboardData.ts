...
export const generateTopProductsData = (period: PeriodType = '30days') => {
    ...
    const multiplier = period === 'year' ? 12 : period === 'month' ? 1 : period === '7days' ? 0.25 : 1;
...
export const generateOrdersData = (period: PeriodType = '30days') => {
    ...
    return [
        { name: 'Completados', value: Math.round((60 + Math.random() * 40) * multiplier), color: '#10b981' },
        { name: 'Pendientes', value: Math.round((20 + Math.random() * 30) * multiplier), color: '#f59e0b' },
        { name: 'En Proceso', value: Math.round((10 + Math.random() * 20) * multiplier), color: '#3b82f6' },
        { name: 'Cancelados', value: Math.round((5 + Math.random() * 10) * multiplier), color: '#ef4444' }
    ];
};
