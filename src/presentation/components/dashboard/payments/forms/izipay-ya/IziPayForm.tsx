...
export default function YapeQRForm({ nameId }: { nameId: string }) {
	const { form, loading, handleChange, createPayment, updatePayment, isUpdate, setForm } =
		usePaymentForm(nameId);

	return (
		<div className="w-full p-0">
			<Card
				shadow="none"
				className="h-full border-1 border-sky-200/30 bg-sky-600/40 w-[100%]"
				isBlurred
			>
				<CardBody className="flex flex-row gap-3">
					<div className="flex flex-col gap-3 w-full">
						<Input
							label="Nombre"
							placeholder="Ej: Paga con tarjetas Debito/Credito"
							value={form.name ?? ''}
						...
						<Input
							label="Merchant ID"
							placeholder="Ej: 90001000"
							value={form.credentials?.merchantId || ''}
							onChange={(e) => handleChange(e, 'credentials', 'merchantId')}
							classNames={{
								label: 'text-black/50 dark:text-white/90',
								innerWrapper: 'bg-transparent',
								input: [
						...
						<Input
							label="Public Key"
							placeholder="Ej: hM6Ytfn8AoTGB5nM"
							value={form.credentials?.publicKey ?? ''}
							onChange={(e) => handleChange(e, 'credentials', 'publicKey')}
							classNames={{
								label: 'text-black/50 dark:text-white/90',
								innerWrapper: 'bg-transparent',
								input: [
						...
						<Input
							className="hidden"
							label="URL QR"
							placeholder="Ej: https://media.qr.com"
							value={form.details?.urlQR ?? ''}
							onChange={(e) => handleChange(e, 'details', 'urlQR')}
						/>
