...
const OrderDetails: React.FC<{ orderId: string }> = ({ orderId }) => {
	...
	const subtotal = orderData.products.reduce(
		(sum: number, product: any) => sum + product.valid_price * product.qty,
		0
	);
									...
									<div className="flex gap-4">
										...
										<div className="flex-1 min-w-0">
											...
											<div className="flex flex-wrap gap-4 text-sm text-zinc-500 dark:text-zinc-400">
												...
												<span>
													Precio: {orderData.currency === 'PEN' ? 'S/' : '$'}{' '}
													{product.valid_price.toFixed(2)}
												</span>
											</div>
										</div>
										<div className="text-right">
											<p className="font-bold text-zinc-900 dark:text-white">
												{orderData.currency === 'PEN' ? 'S/' : '$'}{' '}
												{(product.valid_price * product.qty).toFixed(2)}
											</p>
										</div>
									</div>
								</div>
						...
						<div className="space-y-3">
							...
							{orderData.clientInfo.number_doc && (
								<div className="flex items-center gap-2">
									...
									<p className="text-sm text-zinc-600 dark:text-zinc-400">
										DNI: {orderData.clientInfo.number_doc}
									</p>
								</div>
							)}
						</div>
