...
import { searchableItems, categoryLabels, type SearchableItem } from '@/src/presentation/data/searchableItems';
    ...
    return (
        <div className="relative w-full max-w-md hidden md:block">
            ...
            <div className="relative group">
                <Search
                    size={16}
                    className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors ${isOpen ? 'text-primary' : 'text-zinc-400 group-focus-within:text-primary'
                        }`}
                />
                <input
                    ref={inputRef}
                                        ...
                                        {items.map((item, itemIndex) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center gap-3 px-4 py-2.5 transition-all duration-150 group ${isSelected
                                                ...
                                                >
                                                    ...
                                                    <ArrowRight
                                                        size={14}
                                                        className={`shrink-0 transition-transform ${isSelected ? 'translate-x-0.5' : ''
                                                            }`}
                                                    />
                                                </button>
                                            );
                                ...
                                <div className="px-4 py-2 mt-2 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
                                    <div className="flex items-center gap-3">
                                        <span className="flex items-center gap-1">
                                            <kbd className="px-1 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded text-[10px]">↑↓</kbd>
                                            Navegar
                                        </span>
                                        <span className="flex items-center gap-1">
                                            <kbd className="px-1 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded text-[10px]">↵</kbd>
                                        ...
                                        <span className="flex items-center gap-1">
                                            <kbd className="px-1 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded text-[10px]">Esc</kbd>
                                            Cerrar
                                        </span>
                                    </div>
                                </div>
