...
export default function DashboardError({
	...
	const isAuthError =
		error.message?.includes('AUTH_ERROR_SESSION_EXPIRED') ||
			...
			<div className="relative w-full max-w-lg">
				...
				<div className="relative bg-white/80 dark:bg-dark-card/80 backdrop-blur-xl border border-white/20 dark:border-zinc-800 rounded-3xl shadow-2xl p-8 lg:p-12 overflow-hidden text-center">
					...
					<div className="flex justify-center mb-8">
						<div
							className={`p-6 rounded-3xl shadow-lg transition-transform hover:scale-110 duration-500 ${isAuthError ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 shadow-amber-200/50' : 'bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 shadow-rose-200/50'}`}
						>
							{isAuthError ? (
					...
					{process.env.NODE_ENV === 'development' && (
						<div className="mb-10 p-4 bg-zinc-100/50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 text-left overflow-hidden">
							<p className="text-xs font-mono text-zinc-500 dark:text-zinc-500 truncate mb-1">
								<span className="font-bold text-zinc-700 dark:text-zinc-300">Digest:</span>{' '}
								{error.digest || 'N/A'}
							</p>
							<p className="text-xs font-mono text-rose-600 dark:text-rose-400 break-words">
								<span className="font-bold text-zinc-700 dark:text-zinc-300">Error:</span>{' '}
								{error.message}
							</p>
						</div>
					)}
				...
				<p className="text-center mt-8 text-sm text-zinc-500 dark:text-zinc-500 font-medium">
					¿Necesitas ayuda?{' '}
					<a href="#" className="underline text-primary hover:text-primary-hover">
						Contacta a soporte
					</a>
				</p>
			</div>
