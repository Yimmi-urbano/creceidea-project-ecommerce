...
export default function DashboardPage() {
	...
	const categorySalesData = useMemo(
		() => generateCategorySalesData(selectedPeriod),
		[selectedPeriod]
	);
	...
	return (
		<div className="space-y-6 animate-in fade-in duration-500">
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
				<div>
					<h1 className="text-2xl font-bold tracking-tight mb-1">Dashboard Analytics</h1>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 rounded-2xl border bg-gradient-to-br from-primary/5 to-transparent border-primary/20">
				<div className="text-center">
					<p className="text-sm text-zinc-500 dark:text-zinc-400 mb-1">Productos Más Vendido</p>
					<p className="text-lg font-bold text-zinc-900 dark:text-zinc-100">
						{topProductsData[0]?.name}
					</p>
					<p className="text-xs text-primary">{topProductsData[0]?.sales} unidades</p>
				</div>
				<div className="text-center border-x border-zinc-200 dark:border-zinc-800">
					<p className="text-sm text-zinc-500 dark:text-zinc-400 mb-1">Mejor Día de Ventas</p>
					<p className="text-lg font-bold text-zinc-900 dark:text-zinc-100">
					...
					<p className="text-xs text-primary">
						S/{' '}
						{
							salesData.reduce((max, day) => (day.sales > max.sales ? day : max), salesData[0])
								.sales
						}
					</p>
				</div>
				<div className="text-center">
					<p className="text-sm text-zinc-500 dark:text-zinc-400 mb-1">Tasa de Éxito</p>
					<p className="text-lg font-bold text-zinc-900 dark:text-zinc-100">
						{Math.round(
							((ordersData.find((o) => o.name === 'Completados')?.value || 0) /
								ordersData.reduce((sum, o) => sum + o.value, 0)) *
								100
						)}
						%
					</p>
					<p className="text-xs text-green-600 dark:text-green-400">Pedidos completados</p>
				</div>
			</div>
		</div>
	);
